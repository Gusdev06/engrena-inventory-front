...
import type { CreateDayOffProps, DayOff } from "@/services/entrena-inventory-api/types";
...
export default function DaysOffPage() {
  ...
  const {
    ...
    mutationKey: ["apiInventory.createDayOff"],
    mutationFn: async (dayOffProps: CreateDayOffProps) => {
      const response = await apiInventory.createDayOff({ ...dayOffProps });
      return response;
    },
    onSuccess: () => {
      console.log("foi");
  ...
  const { data: getDaysOffData } = useQuery({
    queryKey: ["apiInventory.getDaysOff"],
    queryFn: async () => {
      const response = await apiInventory.getDaysOff();
